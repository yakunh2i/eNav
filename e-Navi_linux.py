...
def main():
	...
	while True:
		...
		try:
			...
			logger.error('Problem during login to ' + user +'. Will retry login in later.' + '\n')
			time.sleep(300) 			
			break
			#sys.exit(2)
		...
		for emailid in msgs:
			...
			#Read the raw requirement file directly from the .zip & .gzip attachments, no unzipping/extraction
			for filename in SMTP_send.getfiles(svdir):
				fullfilename = os.path.join(svdir,filename)
				fullfilename = os.path.abspath(fullfilename)
				...
				if(fullfilename.endswith('.zip')):	
					try:
						logger.info('Reading the zipped file: ' + fullfilename)
						zfile = zipfile.ZipFile(fullfilename, 'r')
				...
				if(fullfilename.endswith('.gzip')):			
					try:
						logger.info('Reading the gzipped file: ' + fullfilename)
			...
			#for response_part in flag_data:
			#	print '\nFlag is: ' + str(imaplib.ParseFlags(response_part))
			typ, flag_data = mail.store(emailid, '-FLAGS', '\SEEN')
			typ, flag_data = mail.store(emailid, '+FLAGS', '\UNSEEN')
